...
**Cron status (2026-09-07):** only staging's entry is installed. Both entries went in on 2026-09-06, but staging's next real target hit a real outage the old `send-keys` relaunch could not recover from (fixed in #146 — see `notes/deploy-staging-plan.md` Gate F for the full incident and fix timeline); both entries were pulled by hand before production's poll could hit the same failure. With the fix merged to `test` (`257173d`), staging's entry alone was reinstalled and reverified against a real unattended tick. Production's entry stays out until `test` releases to `main` and the same verification is repeated there — the `main/app` checkout does not have the fix yet. The Slack failure-alert webhook (`/etc/muscat-db/slack-webhook-url`) is also still not installed, so a failed deploy currently logs `FAILED` but does not page anyone.
