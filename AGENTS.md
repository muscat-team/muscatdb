...
* muscat.db is updated daily via a cronjob (see [Cron (daily)](README.md#cron-daily) in the README for the crontab entry itself)
...
* use conda env `prose` ($HOME/miniconda3/envs/prose) when running run_photometry
...
* maintain design consistency across all pages based on styles.css
...
* check background processes; report any idle or background processes related to muscat-db before running a new one
* all one-off scripts should live in $HOME/temp, but useful scripts should be kept in the repo
* the server lives inside tmux session `muscatdbgui`. If you (the agent) restart muscat-db yourself, do it inside that same session.
* `--reload` only watches Python files, not Jinja2 templates -- remind the user a restart is needed to see HTML/JavaScript changes
...
CONTRIBUTING.md has the human-contributor-facing version of this section
(setup, pre-PR checklist, the same branch rules in plain form). Keep both in
sync when either changes; this section keeps the mechanics CONTRIBUTING.md
deliberately leaves out -- ruleset internals, review-dismissal behavior, and
the specific incidents that motivated each rule.
...
* an issue therefore closes at the test merge, before the fix is deployed. nothing deploys on a merge to test, and nothing deploys on a push to main either: production moves only when the deploy host takes the released commit, by hand today and from the `deploy/pull-deploy.sh` cron once #131's entries are installed. write `Refs #N` instead and close by hand at release when an issue should outlive the merge
* main is the release branch, but merging to it deploys nothing. production is pinned with `git reset --hard origin/main` in the deploy checkout, run by hand today and from `deploy/pull-deploy.sh` on cron once #131's entries are installed. the old `deploy.yml` push deploy was deleted in #133: it ran on GitHub-hosted runners, which cannot reach ut2 without the VPN, so it had never once worked
...
* when you want a change on the other person's PR, put it in the review and let them implement. do not open a PR against their branch, that is stacking, and do not push to it: your own approval cannot cover your own push, and github refuses an approval from the PR author
